...
func (r *MCPReconciler) setMCPServerRegistrationStatus(ctx context.Context, mcpGatewayExtNS string, mcpsr *mcpv1.MCPServerRegistration, serverID string) error {
...
func (r *MCPReconciler) buildMCPServerConfig(ctx context.Context, targetRoute *gatewayv1.HTTPRoute, mcpsr *mcpv1.MCPServerRegistration) (*config.MCPServer, error) {
	...
	userSpecificListEnabled := false
	if ann, ok := mcpsr.Annotations["mcp.kuadrant.io/experimental-discovery"]; ok && strings.Contains(ann, `"userSpecificList":"Enabled"`) {
		userSpecificListEnabled = true
	}
	...
	if ann, ok := mcpsr.Annotations["mcp.kuadrant.io/experimental-discovery"]; ok {
		var expData struct {
			TokenURLElicitation *config.TokenURLElicitationConfig `json:"tokenURLElicitation,omitempty"`
		}
		if err := json.Unmarshal([]byte(ann), &expData); err == nil && expData.TokenURLElicitation != nil {
			serverConfig.TokenURLElicitation = expData.TokenURLElicitation
		}
	}
